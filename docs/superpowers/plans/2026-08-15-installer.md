...
**Goal:** Add a POSIX `install.sh` that downloads and installs the correct `pebblehost` release binary for Linux, macOS, and WSL.

## Global Constraints

- No project binary behavior changes; only add `install.sh` and README docs.
...
- CI lint: add `shellcheck install.sh` step if shellcheck is available; otherwise a basic `bash -n` syntax check.
...
- Output: installs `pebblehost` binary, prints version and path.

- [ ] **Step 1: Detect target triple**
...
- [ ] **Step 3: Download and extract**
  ...
  Download to a temp directory, extract `pebblehost` (or `pebblehost.exe` on Windows, but script targets POSIX).

- [ ] **Step 4: Choose install prefix**
  Default: `~/.local/bin` if it exists or is creatable; else `/usr/local/bin` if writable; else print a `sudo` hint.
  Honor `--prefix`.

- [ ] **Step 5: Install binary and verify**
  Use `cp`/`chmod +x` to place binary. Run `pebblehost --version` unless `--skip-version-check`.
...
### Task 3: Add CI lint for `install.sh`

**Files:**
- Modify: `.github/workflows/lint.yml`

- [ ] **Step 1: Add shellcheck or syntax check job step**
  Install shellcheck if missing, run `shellcheck install.sh` or `bash -n install.sh`.
...
- [ ] **Step 1: Syntax check**
  Run `shellcheck install.sh` locally if available; otherwise `bash -n install.sh`.

- [ ] **Step 2: Dry-run test**
  Run the script with `--prefix /tmp/pebblehost-test` and a specific tag, verify binary installs and `pebblehost --version` works.

- [ ] **Step 3: Run existing Rust tests/lint**
...
- [ ] **Step 1: Add `install.sh`, README, lint.yml, plan, spec**
